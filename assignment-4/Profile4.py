...
class ReceivedMessage(dict):
    def __init__(self, msg:str = None, timestamp: float = 0):
        self._timestamp = timestamp
        self.set_user_msg(msg)
        dict.__init__(self, msg=self._msg, timestamp=self._timestamp)
    
    def set_user_msg(self, msg):
        self._msg = msg 
        dict.__setitem__(self,'msg', msg)
    ...
    """ 
    entry = property(get_msg, set_user_msg)
    timestamp = property(get_time, set_time)

class SentMessages(dict):
    def __init__(self, msg:str = None, timestamp: float = 0):
        self._timestamp = timestamp
        self.set_user_msg(msg)
        dict.__init__(self, sent_msg=self._msg, timestamp=self._timestamp)
    
    def set_user_msg(self, msg):
        self._msg = msg 
        dict.__setitem__(self,'sent_msg', msg)
...
class Profile:
    ...
    def load_profile(self, path: str) -> None:
        ...
        if p.exists() and p.suffix == '.dsu':
            try:
                ...
                for user, msg in obj['_received'].items():
                    self._received[user] = msg
                for user, msg in obj['_sent'].items():
                    self._sent[user] = msg
                f.close()
            except Exception as ex:
    ...
    def add_message(self, message: ReceivedMessage, sender: str):
        if sender in self._received.keys():
            self._received[sender].append(message)
        elif sender not in self._received.keys():
            self._received[sender] = [message]

    def add_sent_message(self, message: SentMessages, recipient: str):
        if recipient in self._sent.keys():
            self._sent[recipient].append(message)
        elif recipient not in self._sent.keys():
            self._sent[recipient] = [message]
